...
static void setup_uinput_device() {
	...
	if(ioctl(uifd, UI_SET_EVBIT, EV_KEY) < 0)
		die("error: ioctl");
	if(ioctl(uifd, UI_SET_KEYBIT, KEY_SPACE) < 0)
		die("error: ioctl");

	memset(&uidev, 0, sizeof(uidev));
	snprintf(uidev.name, UINPUT_MAX_NAME_SIZE, "uinput-keystroke-simulator");
...
static void send_keystroke() {
	memset(&ev, 0, sizeof(struct input_event));
	ev.type = EV_KEY;
	ev.code = KEY_SPACE;
	ev.value = 1;
	if(write(uifd, &ev, sizeof(struct input_event)) < 0)
		die("error: write");
...
static void close_uinput_device() {
...
static char const * vu(unsigned channel)
{
  ...
  //printf("\rinput level: dB[%d]:%f\n", channel, dB);
  if(dB > DB_THRESHHOLD && dB_lock == 0) {
